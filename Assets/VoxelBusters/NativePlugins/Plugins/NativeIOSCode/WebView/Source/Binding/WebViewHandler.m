...
- (UnityWebView *)createWebViewWithFrame:(CGRect)frame andTag:(NSString*)tag
{
    ...
    if (webview == NULL)
    {
        UnityWebView *webview    = [UnityWebView CreateWithFrame:frame tag:tag];
		NSLog(@"[WebViewHandler] creating webview with tag: %@ frame: %@", tag, NSStringFromCGRect(frame));
...
- (void)setFrame:(CGRect)frame forWebViewWithTag:(NSString *)tag
{
    UnityWebView *webview    = [self getWebViewWithTag:tag];
	...
	NSLog(@"[WebViewHandler] setFrame: %@", NSStringFromCGRect(frame));
