...
static VerificationController 	*singleton				= NULL;
static __strong NSString		*SharedSecretKey		= NULL;
static __strong NSString		*VerifyUsingServerURL	= NULL;
...
+ (void)VerifyUsingURL:(NSString *)serverURL sharedSecret:(NSString *)sharedSecretKey
{
	VerifyUsingServerURL	= serverURL;
...
- (id)init
{
	...
	if (self != nil)
    {
        URLConnectionCachedData 				= [[NSMutableDictionary alloc] init];
...
// Call it with the SKPaymentTransaction.transactionReceiptInNSDataFormat
...
{    
	...
	{
		NSLog(@"[VerficationController] transaction is failed, so marking it as verified successfully");
	...
	NSData *receiptData			= [transaction transactionReceiptInNSDataFormat];
    NSString *receiptDataStr	= [self encodeBase64:(uint8_t *)receiptData.bytes
	...
	{
		payload	= [NSString stringWithFormat:@"{\"receipt-data\" : \"%@\"}", receiptDataStr];
	}
	else
	{
		payload	= [NSString stringWithFormat:@"{\"receipt-data\" : \"%@\", \"password\" : \"%@\"}", receiptDataStr, SharedSecretKey];
	}
	
	NSData *payloadData = [payload dataUsingEncoding:NSUTF8StringEncoding];
	
	// First verify using production server
	NSString *serverURL	= IsNullOrEmpty(VerifyUsingServerURL) ? ITMS_PROD_VERIFY_RECEIPT_URL : VerifyUsingServerURL;
	
	[self sendVerifyRequest:serverURL
			withPayloadData:payloadData
		   completionHadler:completionHandler];
}

#define kCompletionHandler 	@"kCompletionHandler"
#define kPayloadData		@"kPayloadData"

- (void)sendVerifyRequest:(NSString*)serverURL
		  withPayloadData:(NSData *)payloadData
		 completionHadler:(VerifyCompletionHandler)completionHandler
{
	...
	// Cache required informations for future reference
	id key								= [NSValue valueWithNonretainedObject:connection];
	NSMutableDictionary *requestData	= [[NSMutableDictionary alloc] init];
	[requestData setObject:completionHandler forKey:kCompletionHandler];
	[requestData setObject:payloadData forKey:kPayloadData];
	
	[URLConnectionCachedData setObject:requestData forKey:key];
...
{
    if (!(transaction && transaction.transactionReceiptInNSDataFormat && [transaction.transactionReceiptInNSDataFormat length] > 0))
    {
    ...
    NSDictionary *receiptDict       	= [self dictionaryFromPlistData:transaction.transactionReceiptInNSDataFormat];
...
- (bool)retryUsingSandboxServer:(NSDictionary*) verifiedReceiptDictionary
{
...
- (BOOL)doesTransactionInfoMatchReceipt:(NSDictionary*) verifiedReceiptDictionary
{
    // Check the status of the verifyReceipt call
    id status = [verifiedReceiptDictionary objectForKey:@"status"];
    if (!status)
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
	...
	// Use cached data for this request
	id key										= [NSValue valueWithNonretainedObject:connection];
	VerifyCompletionHandler completionHandler	= [[URLConnectionCachedData objectForKey:key] objectForKey:kCompletionHandler];
    completionHandler(FALSE);
	
	// Remove cached data
	[URLConnectionCachedData removeObjectForKey:key];
}

- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data
{
    NSDictionary *verifiedReceiptDictionary 	= [self dictionaryFromJSONData:data];
	
	// Use cached data for this request
	id key										= [NSValue valueWithNonretainedObject:connection];
	NSMutableDictionary *cachedData				= [URLConnectionCachedData objectForKey:key];
	VerifyCompletionHandler completionHandler 	= [cachedData objectForKey:kCompletionHandler];
	NSData *payloadData							= [cachedData objectForKey:kPayloadData];
	
	// Remove cached data
	[URLConnectionCachedData removeObjectForKey:key];
	
	// Check if we need to request sandbox server
	if (IsNullOrEmpty(VerifyUsingServerURL) && [self retryUsingSandboxServer:verifiedReceiptDictionary])
	{
		[self sendVerifyRequest:ITMS_SANDBOX_VERIFY_RECEIPT_URL
				withPayloadData:payloadData
			   completionHadler:completionHandler];
	}
	else
	{
   	 	...
   	 	BOOL isOk = [self doesTransactionInfoMatchReceipt:verifiedReceiptDictionary];
